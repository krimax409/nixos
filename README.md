...
- `configs/quickshell/win11-start-menu/defaults.json` задаёт содержимое Win11-меню:
  pinned-приложения, быстрые действия и power-меню. На главном экране после
  закреплённых приложений показывается полный список приложений.
  Этот JSON можно редактировать напрямую; состояние pin/recent хранится отдельно
  в `$XDG_STATE_HOME/win11-start-menu/state.json`.

В открытом меню поле поиска сразу получает фокус. `Ctrl+L` возвращает фокус в
поиск, `Enter` запускает первый результат, `Down` переводит фокус в список, а
`Escape` сначала очищает запрос и только повторное нажатие закрывает меню.
Клик вне карточки меню закрывает его.
ПКМ по приложению открывает контекстные действия: запуск, закрепление или
открепление, а для pinned-приложений также перемещение влево или вправо.
